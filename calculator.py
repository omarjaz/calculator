...
st.markdown("<h1 style='text-align: center;'>Calculadora de Ventas</h1>", unsafe_allow_html=True)

st.markdown("---")
st.markdown("\n\n")
# 2. INPUTS
num_beers = st.radio("**Cervezas** 🍺", list(range(0, 16)), horizontal=True)
num_bottles_water = st.number_input("Botellas de agua 💧", min_value=0, value=0, step=1)
num_refrescos = st.number_input("Refrescos: 🥤", min_value=0, value=0, step=1)
num_coctels = st.number_input("Copas 🍸", min_value=0, value=0, step=1)
